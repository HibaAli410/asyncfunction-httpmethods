...
function presentingTodo (task, id) {
  ...
  li.addEventListener('dragstart', function (event) {
    event.dataTransfer.setData('dragetask', id)
  })
  ...
  li.addEventListener('drop', function (event) {
    event.preventDefault()
    const dragedId = event.dataTransfer.getData('dragetask')
    const targetId = li.dataset.todo // data-todo attribute get through dataset.todo
    // element.dataset.todo any element like ul li span etc
    let todolist = JSON.parse(localStorage.getItem('todolist'))

    const drageIndex = todolist.findIndex(item => item.id === dragedId)
    const targetIndex = todolist.findIndex(item => item.id === targetId)
    //splice not delete a key ,its value of an array its reture a peice of array
    //so its store as like this [] = splice
    //  when i just store in array as a variable its breaks my todo structure like [a,[b],c]
    //[] = splice this is array destruction
    //splice returns array
    //we only want the single object inside it
    const [drageli] = todolist.splice(drageIndex, 1)
    todolist.splice(targetIndex, 0, drageli)
    localStorage.setItem('todolist', JSON.stringify(todolist))
    ul.innerHTML = ''
    todolist.forEach(todo => presentingTodo(todo, todo.id))
  })
  ...
  removeli.addEventListener('click', function (event) {
    ...
    let confirmation = confirm(
      'Are You Sure to delet it Without Compete the task'
    )

    let todolist = JSON.parse(localStorage.getItem('todolist'))
    let index = todolist.findIndex(todo => todo.text === task)
    if (confirmation === true && todolist[index].completed === false) {
      li.remove()
