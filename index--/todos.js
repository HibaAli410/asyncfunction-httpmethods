...
function presentingTodo(task, index) {
    // console.log(task);
    task = task.text;
    const li = document.createElement('li');
    li.dataset.id = index; // store index 
    ...
    //checkbox, styling
    checkbox.classList.add("w-4", "h-4", "border-4", "border-red-200", "accent-red-500", "hover:bg-violet-600", "focus:outline-2", "focus:outline-offset-2", "focus:outline-red-800");
...
if (buttonvisibilty && buttonvisibilty.length > 0) {
    ...
    deleteselected.addEventListener('click', function () {
        ...
        itemlist.forEach((element, position) => {
            console.log(position);
            const checkbox = element.querySelector('input[type="checkbox"]');
            const span = li.querySelector('span').innerText;
            const index = todolist.findIndex(item => item === span);
...
function todoList(event) {
    ...
    if (event.keyCode === 13 && event.target.value.trim() !== "") {
        ...
        localStorage.setItem('todolist', JSON.stringify(todolist));
        presentingTodo(todoObj, todolist.length - 1);
        event.target.value = "";
...
fetchtodos.forEach((element, index) => {
    presentingTodo(element, index);
});
